 ...
 */
class LocalDomainFactoryV9 implements DomainFactoryInterface
{
    /**
     * @var Connection
     */
    ...
    /**
     * @var LocalDomainFactoryV8
     */
    protected $localDomainFactoryV8 = null;
    ...
    public function __construct()
    {
        ...
        $this->localDomainFactoryV8 = GeneralUtility::makeInstance(LocalDomainFactoryV8::class);
    }
    ...
    public function getDomainForRecord(int $uid, string $table = 'pages'): string
    {
        ...
        if ($uri = $this->localDomainFactoryV8->getDomainForRecord($uid, $table)) {
            return $uri;
        }
        ...
        if ('pages' !== $table) {
            ...
            $pageIdentifier = $query->select('*')
                                    ...
                                    ->execute()
                                    ->fetch();
        } elseif ($sysLanguage > 0) {
            $parentField = $this->tcaService->getTransOrigPointerField($table);
            $query = $this->connection->createQueryBuilder();
            $pageIdentifier = $query->select('uid')
                                    ->from('pages')
                                    ->where('uid=' . (int)$recordRow[$parentField])
                                    ->execute()
                                    ->fetchColumn(0);
        } else {
            $pageIdentifier = $uid;
